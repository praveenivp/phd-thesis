...
\begin{figure}[H]
	...
	\caption{Vessel distance versus signal change for GE-EPI and bSSFP acquisition. (A) Vessel distance (in mm) from large veins (segmented using a Frangi filter on a high-resolution Cartesian GE-scan; TE=16 ms, 0.4 mm isotropic resolution) is shown alongside exemplary anatomical axial images acquired with GE-EPI (0.8 mm isotropic resolution) and spiral-bSSFP (0.8 mm isotropic resolution). Corresponding signal change in significantly activated voxels from the activation maps (B and C) is plotted against vessel distance in (D) and (E), respectively. (F) A similar plot is shown for a bSSFP functional experiment with TR=6 ms and 1 mm isotropic voxels.}
